...
{
    public const int C_PageRow = 9;
    public const int C_PageColumn = 11;
    ...
    [HideInInspector]
    ///pagecoords that board used to move to,last coord will be the last page of level
    private List<Vector2> m_ListPageCoords;
    
    protected override void Init()
    {
        base.Init();
        m_ListPageCoords = new List<Vector2>();
    }

    public void LoadLevel()
    {
        var jsonString = March.Core.ResourceManager.ResourceManager.instance.Load<TextAsset>(Configure.BundleLevel, "" + level);
        if (jsonString == null)
        {
    ...
    public void LoadLevel(string json)
    {
        ...
        //add page coord if have
        if (properties.ContainsKey("page_coord"))
        {
            var coordlist = ((string)properties["page_coord"]).Split('|').ToList();
            for (int j = 0; j < coordlist.Count; ++j)
            {
                Vector2 coord = new Vector2();
                var coordSingle = coordlist[j].Split(',').ToList();

                coord.x = int.Parse(coordSingle[0]);
                coord.y = int.Parse(coordSingle[1]);
                m_ListPageCoords.Add(coord);
            }
        }
    ...
    TILE_TYPE DataToTileType(int key)
    {
        ...
        switch (data)
        {
            ...
            case "pass_through_tile":
                return TILE_TYPE.PASSTHROUGH;
            case "light_tile":
                return TILE_TYPE.LIGHT;
            case "dark_tile":
                return TILE_TYPE.DARK;
        }
    ...
    /// <summary>
    /// if have multiple page
    /// </summary>
    /// <returns>ture or false</returns>
    public bool HaveMultiplePage()
    {
        return m_ListPageCoords.Count > 1;
    }

    /// <summary>
    /// return coordinate of page index
    /// </summary>
    /// <returns></returns>
    public Vector2 GetPageCoord(int index)
    {
        if (m_ListPageCoords.Count > 0 && index < m_ListPageCoords.Count)
        {
            return m_ListPageCoords[index];
        }
        //default is center of max row and column
        return new Vector2(C_PageRow / 2, C_PageColumn / 2);
    }
