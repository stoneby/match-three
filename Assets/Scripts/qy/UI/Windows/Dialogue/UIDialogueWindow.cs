...
public class UIDialogueWindow : UIWindowBase {
    ...
    private void ShowStory(StoryItem story)
    {
        ...
        {
            BackGroundColor = Color.black;
            var sp = March.Core.ResourceManager.ResourceManager.instance.Load<Sprite>(Configure.BundleStoryBackground,curDialogue.bgFile);
            if (sp != null)
            {
                imgBG.sprite = sp;
                imgBG.DOFade(1, 0.5f);
        ...
        string personFile = curDialogue.personFile;
    ...
    private UIDialoguePerson GetPerson(string name,string position)
    {
        ...
        if(personGO==null)
        {
            GameObject go = March.Core.ResourceManager.ResourceManager.instance.Load<GameObject>(Configure.BundleStoryDialog, name);
